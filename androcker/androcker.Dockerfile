...
RUN echo "deb http://kali.download/kali kali-last-snapshot main contrib non-free" > /etc/apt/sources.list && \
	...
	DEBIAN_FRONTEND=noninteractive apt-get install -y nano curl wget unzip git netcat python2 python3 python3-pip p7zip-full p7zip-rar p7zip unzip net-tools build-essential iputils-ping pciutils bash-completion apt-transport-https software-properties-common ca-certificates openjdk-11-jdk openjdk-11-jre jadx dex2jar terminator && \
